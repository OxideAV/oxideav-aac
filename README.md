...
| Feature                                | Status                                  |
|----------------------------------------|-----------------------------------------|
| Object types                           | AAC-LC (`AOT 2`) decode + encode; AAC-LTP (`AOT 4`) decode (long-window SCE / CPE / LFE through the LC `raw_data_block()` path with §4.6.7 long-term prediction; round-108); AAC-LD (`AOT 23`) decode of `er_raw_data_block()` for channelConfiguration 1 (mono) and 2 (stereo, common-window or independent ICS) at 512- and 480-sample frame lengths, including LTP — multichannel LD + low-overlap window pending; AAC-ELD (`AOT 39`) ASC parse only |
...
| LD `er_raw_data_block()` decode        | Round-95 bootstrap: AOT 23 (`AOT_ER_AAC_LD`) recognised in the ASC and dispatched to `decode_packet_ld` per §4.4.2.3 Table 4.19; SCE (channelConfiguration 1) and CPE (channelConfiguration 2, common-window or independent-ICS) elements decoded through `decode_ics_ld` + `decode_spectrum_long_with_swb` (LD-aware spectrum decoder reading `ld_eld::SWB_LD_512` / `SWB_LD_480`) + per-channel `imdct_and_overlap_ld`. PNS / IS / M/S / TNS apply on the LD spectrum the same way as the LC long path (codebook semantics are identical; §4.6.17 only changes the SWB layout and IMDCT size). Round-101 adds long-term prediction (§4.6.7): `ltp_data()` is parsed in `ics_info` (10-bit `ltp_lag` with previous-frame reuse via `ltp_lag_update`, 3-bit `ltp_coef` → Table 4.98, per-sfb `ltp_long_used`) and the single-tap predictor `x_est(i) = ltp_coef · x_rec(i − N/2 − ltp_lag)` (M = N/2 for ER AAC LD) is windowed-MDCT'd and added to the residual on enabled bands, slotting between PNS and TNS in the §4.1.1.1 Figure 4.2 tool chain with PNS/IS bands skipped per §4.6.7.4.2. Multichannel LD (channelConfiguration 3..=7), short-window LTP, low-overlap window shape, and LD-SBR are deferred. |
