...
| Feature                                | Status                                  |
|----------------------------------------|-----------------------------------------|
| Object types                           | AAC-LC (`AOT 2`) decode + encode; AAC-LD (`AOT 23`) decode of `er_raw_data_block()` for channelConfiguration 1 (mono) and 2 (stereo, common-window or independent ICS) at 512- and 480-sample frame lengths, now including long-term prediction (LTP, §4.6.7) — multichannel LD + low-overlap window pending; AAC-ELD (`AOT 39`) ASC parse only |
...
| Long term prediction (LTP, §4.6.7)     | AAC-LD (`AOT 23`) long-window LTP decoded (see LD row). Non-LD AAC-LTP (`AOT 4`) decoder tools landed in round-104: `ics::parse_ltp_data` reads the Table 4.49 non-LD branch (unconditional 11-bit `ltp_lag` 0..2047, no `ltp_lag_update` reuse; 3-bit `ltp_coef`; long-window `ltp_long_used[]` for exactly `max_sfb` flags per the Table 4.49 loop bound, or the short-window `ltp_short_used` / `ltp_short_lag` ±8 nest), and `synth::apply_ltp` runs the long-window single-tap predictor `x_est(i) = ltp_coef · x_rec(i − M − ltp_lag)` with `M = 0` (vs `M = N/2` for LD), `N = 2048`, reading `x_rec(i<0)` from a new per-channel `ChannelState.ltp_hist` ring, long-windowing + forward-MDCT'ing the predicted signal, and adding `X_est` to the residual on enabled bands (PNS / IS skipped per §4.6.7.4.2). The AOT 4 full frame-decode wiring (`make_decoder` still gates AOT 4) and short-window predictor synthesis are deferred. |
