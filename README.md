...
| Feature                                | Status                                  |
|----------------------------------------|-----------------------------------------|
| Object types                           | AAC-LC (`AOT 2`) decode + encode; AAC-LTP (`AOT 4`) decode (long-window SCE / CPE / LFE through the LC `raw_data_block()` path with §4.6.7 long-term prediction; round-108); AAC-LD (`AOT 23`) decode of `er_raw_data_block()` for channelConfiguration 1..=7 (mono SCE, stereo CPE, 3.0, 4.0, 5.0, 5.1, 7.1) at 512- and 480-sample frame lengths, including LTP — low-overlap window + short-window LTP + LD-SBR pending; AAC-ELD (`AOT 39`) ASC parse only |
...
| LD `er_raw_data_block()` decode        | Round-95 bootstrap: AOT 23 (`AOT_ER_AAC_LD`) recognised in the ASC and dispatched to `decode_packet_ld` per §4.4.2.3 Table 4.19; round-111 extends the element walk to **channelConfiguration 1..=7** (1 mono SCE, 2 stereo CPE, 3 SCE+CPE 3.0, 4 SCE+CPE+SCE 4.0, 5 SCE+CPE+CPE 5.0, 6 SCE+CPE+CPE+LFE 5.1, 7 SCE+CPE+CPE+CPE+LFE 7.1) — Table 4.19 fixes the element sequence by channel config (no `id_syn_ele` dispatch as in LC), so `ld_element_layout()` enumerates the SCE/CPE/LFE sequence and drives per-element `decode_sce_ld` / `decode_cpe_ld` helpers (LFE shares SCE syntax, long-window-only). Each element decodes through `decode_ics_ld` + `decode_spectrum_long_with_swb` (LD-aware spectrum decoder reading `ld_eld::SWB_LD_512` / `SWB_LD_480`) + per-channel `imdct_and_overlap_ld` with PNS / IS / M/S / TNS reusing the LC long-path helpers (codebook semantics are identical; §4.6.17 only changes the SWB layout and IMDCT size). Round-101 adds long-term prediction (§4.6.7): `ltp_data()` is parsed in `ics_info` (10-bit `ltp_lag` with previous-frame reuse via `ltp_lag_update`, 3-bit `ltp_coef` → Table 4.98, per-sfb `ltp_long_used`) and the single-tap predictor `x_est(i) = ltp_coef · x_rec(i − N/2 − ltp_lag)` (M = N/2 for ER AAC LD) is windowed-MDCT'd and added to the residual on enabled bands, slotting between PNS and TNS in the §4.1.1.1 Figure 4.2 tool chain with PNS/IS bands skipped per §4.6.7.4.2 — LTP fires per channel across all multichannel configurations. ChannelConfiguration 0 (PCE-defined) and 8.. (reserved) remain `Error::Unsupported`; short-window LTP, low-overlap window shape, and LD-SBR are deferred. |
...
| Gain control / SSR / Main              | Refused (`Error::Unsupported`)          |
| Long term prediction (LTP, §4.6.7)     | AAC-LD (`AOT 23`) long-window LTP decoded (see LD row). Non-LD AAC-LTP (`AOT 4`) is now decoded end-to-end (round-108): `make_decoder` accepts the AOT 4 AudioSpecificConfig and routes it through the AAC-LC `raw_data_block()` path, where ics_info's `predictor_data_present` body is parsed as `ltp_data()` (Table 4.6 `audioObjectType != 1` branch) via `ics::parse_ics_info_with_ltp` — `ltp_data_present` then the non-LD `ltp_data()` (11-bit `ltp_lag`, 3-bit `ltp_coef` → Table 4.98, per-sfb `ltp_long_used[]`), plus the second channel's `ltp_data()` in the CPE common-window case. `decoder::decode_ics_ltp` threads the side info through SCE / LFE / independent-ICS CPE, and the §4.6.7.3 single-tap predictor `synth::apply_ltp` (`x_est(i) = ltp_coef · x_rec(i − M − ltp_lag)`, `M = 0`, `N = 2048`, reading `x_rec(i<0)` from the per-channel `ChannelState.ltp_hist` ring) windows + forward-MDCTs the predicted signal and adds `X_est` to the residual on enabled bands, slotting between PNS/IS (and M/S in CPE) and TNS per Figure 4.2 with PNS/IS bands skipped (§4.6.7.4.2); each channel's reconstructed PCM is fed back into `ltp_hist` after IMDCT. AOT 4 is long-window-only by construction (Table 4.6 reads `predictor_data_present` only in the long-window arm, so a plain SCE/CPE never carries short-window LTP). Short-window LTP exists only in the scalable profile (unsupported), and `ltp_short_used` / `ltp_short_lag` parsing in `ics::parse_ltp_data` remains available but unwired. |
...
| M/S stereo (§4.6.13)                   | Per-band L/R-vs-M/S decision by bit cost + activity gate (energy-balance ∈ [1/8, 8] AND \|corr\| ≥ 0.4) + magnitude-weighted sign-agreement gate (≥ 55 % per-line polarity agreement). Round-30 layers an ISO/IEC 13818-7 §6.6.1.3 perceptual-entropy arbiter on top: VETOs bit-cost-picked M/S when `pe_ms > pe_lr · 1.25`, PROMOTEs M/S in a 5 %-bit-cost tie when `pe_ms ≤ pe_lr · 0.75`. PE uses Johnston's binaural masking threshold (`min(thr_L, thr_R)`) for both M and S transmissions. Override via env `OXIDEAV_AAC_DISABLE_CPE_PSY_MS=1`. Centred-stereo three-tone fixture (440 + 880 + 1760 Hz triad, 5 % per-channel tilt): R +2.50 dB / L +0.03 dB PSNR at -1.14 % bytes vs round-29 bit-cost-only baseline. |
...
| Gapless playback metadata              | `gapless::GaplessInfo` triple (delay/padding/valid_samples) + Apple iTunSMPB-format string emitter AND parser (`GaplessInfo::parse_itunsmpb`); AAC-LC reports 2112-sample priming, HE-AAC reports 2624 (high rate). End-of-file padding rounded to the next packet boundary so an MP4 `edts/elst` writer or ID3v2 `TXXX:iTunSMPB` wrapper can round-trip the source PCM sample-accurately. Decoder-side: container readers consuming an iTunes-tagged source can parse the canonical 12-word ASCII tag back into a `GaplessInfo` for sample-accurate trim, tolerating multi-space / mixed-whitespace separators, upper/lower-case hex, and non-zero reserved word from third-party taggers. |
