...
## [0.1.8](https://github.com/OxideAV/oxideav-aac/compare/v0.1.7...v0.1.8) - 2026-09-02
...
- `StreamEncoder::with_layout`: PCE-described speaker sets
  (`channelConfiguration = 0`, §8.5.2.2 lists, PCE in every
  `raw_data_block()`), `program_config()` accessor; `make_encoder`
  routes a named non-default `channel_layout` and the 7-channel count
  (6.1) through it
...
- §4.6.12.1 SSR front-half filterbank (`ssr_filterbank`), closing the
  ...
  even-band (2nd/4th ordinal, 0-based 1 and 3) spectral reversal, and
  the per-band 256/32-line IMDCTs under the quarter-scale §4.6.11.3.2
  window geometry (`(N_l, N_s) = (512, 64)`); the arrangement and
  reversal convention are pinned by a tone-placement test against the
  Annex C.2.1.1 analysis PQF
- SSR KBD windows: the Table 4.A.13 (`N = 64`, `α = 6`) and
  Table 4.A.14 (`N = 512`, `α = 4`) Kaiser-Bessel windows generated by
  the §4.6.11.3.2 running-sum construction and validated against the
  normative table listings to print precision
- `ssr::SsrChannelDecoder` — the complete per-channel §4.6.12 SSR
  pipeline (front-half filterbank → gain compensation/overlap → IPQF),
  validated by full round-trip tests against the Annex C.2.1.1
  analysis PQF: steady `ONLY_LONG` and a full window-sequence
  transition chain reconstruct within the PQF pair's
  near-perfect-reconstruction bound (err/sig < 1e-3, both window
  shapes), and §4.6.12.3.2 gain ladders applied encoder-side cancel
  end to end
